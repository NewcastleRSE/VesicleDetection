...
def plot_all_cluster_crops(path, viewer):
    ...
    for crop_file in raw_path.glob('*.h5'):
        ...
        if dset_name in f:
            rawdat = f[dset_name][:]
            option = '_raw'
            viewer.add_image(data=rawdat, name=crop_file.stem+option, blending='additive', colormap='grey', contrast_limits=[rawdat.min(), rawdat.max()])
        f.close()
    for crop_file in masked_path.glob('*.h5'):
        ...
        if dset_name in f:
            maskdat = f[dset_name][:]
            option = '_masked'
            viewer.add_image(data=maskdat, name=crop_file.stem+option, blending='additive', colormap='grey', contrast_limits=[maskdat.min(), maskdat.max()])
        f.close()
