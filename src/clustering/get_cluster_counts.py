...
def count_positive_per_cluster(
    npz_path,
    prediction_path,
    csv_out,
    min_size=None,
    ...
    if min_size is not None:
        valid_clusters = {
            cid for cid in valid_clusters if cluster_sizes[cid] >= int(min_size)
        }
    if max_size is not None:
        valid_clusters = {
            cid for cid in valid_clusters if cluster_sizes[cid] <= int(max_size)
        }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python get_cluster_counts.py <clusters.npz> <prediction.zarr> <output.csv> [--min_size N] [--max_size M] [--use_filenames]"
        )
        sys.exit(1)
    ...
    csv_out = sys.argv[3]
    min_size = (
        sys.argv[sys.argv.index("--min_size") + 1] if "--min_size" in sys.argv else None
    )
    max_size = (
        sys.argv[sys.argv.index("--max_size") + 1] if "--max_size" in sys.argv else None
    )

    pos, neg, tot, what = count_positive_per_cluster(
        npz_path, prediction_path, csv_out, min_size, max_size, use_filenames=False
    )
